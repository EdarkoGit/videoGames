...
export const reqGetMyServer=(param)=>{
  return async function(dispatch){
    if (param) return dispatch(actionsGenerator(SET_VIDEOGAME,await reqAxios(`${URL_VIDEOGAMES}?word=${param}`)))   
...
export const getDetailVideogame =(idVideogame)=>{
  return async function(dispatch){
    dispatch(actionsGenerator(SET_DETAIL_VIDEOGAME, await reqAxios(URL_VIDEOGAMES+idVideogame)))
  }
}