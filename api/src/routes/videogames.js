...
videogames.get('/', async(req,res,next)=>{ 
  ...
  try {
    ...
    const dataRAWG= await getRAWG(`https://api.rawg.io/api/games?key=${API_KEY}&&page_size=40`)
...
videogames.get('/:idVideogame', async(req,res,next)=>{ 
  ...
  try {
    ...
    const dataRAWG= await getRAWG(`https://api.rawg.io/api/games/${idVideogame}?key=${API_KEY}`) //https://api.rawg.io/api/games/4?key=4575ee8ccb974b7db0add89fe707f077
